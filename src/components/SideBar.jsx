...
const SideBar = ({ isNavbarOpen, ToggleNavBar }) => {
  const linkClass = ({ isActive }) =>
    isNavbarOpen
      ? `px-4 flex items-center min-w-[17rem] rounded-r-full  p-3  pl-7 ${
          isActive ? "bg-darkYellow" : "hover:bg-lightGrey"
        }  `
      : `px-4 flex items-center aspect-square rounded-full ml-3 ${
          isActive ? "bg-darkYellow" : "hover:bg-lightGrey"
        }  p-3  `;
  ...
  return (
    <aside className="py-4 sticky top-[4rem] min-h-[calc(100vh-4rem)] z-10 ">
