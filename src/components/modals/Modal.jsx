...
const Modal = ({children, focusOutAble=true, closeAble=true}, ) => {
  ...
  return ( 
    <>
    <button className={` bg-opacity-21 rounded-2xl shadow-lg backdrop-blur-[5px] fixed w-full h-full z-20 inset-0 flex items-center justify-center ${!focused && "hidden"}`} onClick={()=>{if(focusOutAble)setFocused(false)
    }}>
